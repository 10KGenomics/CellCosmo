...
; The current configuration file has 9 sections, the `global` section's
; option can used for all other sections. so you need to config these
; options, some of these parameters have a default assignment mechanism,
; so you can level these options empty such as [
;   global.outdir | global.sample | global.thread | global.genomeDir
; ]; some of these parameters have [required] annotation on themselves,
; which must be configuration, such as [
;   global.fq1 | global.fq2
; ].
;
; There is one thing you need to know:
; the program will use the `global.genomeDir` parameter to determine whether to run
; star index step. More detailed description please see the annotation on themselves.
;
